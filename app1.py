...
# ---------------------------
# Geocoding helper (Nominatim) — used for implicit place lookups
# ---------------------------
...
def get_bbox_for_place(place_name: str, user_agent: str = "ARGO-RAG-Explorer/1.0 (+https://example.com)"):
    ...
    try:
        ...
        pass
    # small fallback for Arabian Sea common queries
...
def _simple_parse_question(question: str) -> Dict[str, Any]:
    ...
    for var in ALLOWED_VAR_SUBSTR + ["salinity", "temperature"]:
        if var in q:
            ...
            break
    # detect DMS/decimal lat-lon simple forms
...
def assemble_mcp_context(index_rows: Optional[pd.DataFrame], nc_previews: Dict[str, pd.DataFrame], chroma_client=None, question: str = "", emb_model=None, top_k: int = 5) -> Dict[str, Any]:
    """Assemble context pieces for LLM: index summary, short .nc previews, and optional vector retrievals.
    Returns a dictionary with 'context_text' and structured 'chunks' for traceability.
    """
    parts = []
    chunks = []
    ...
    # optional vector retrievals using chroma_client if provided
    if chroma_client is not None and emb_model is not None:
        try:
            # build simple text query embedding and ask chroma for nearest metadatas
...
def rag_answer_with_mcp(llm, emb_model, question: str, index_rows: Optional[pd.DataFrame], nc_previews: Dict[str, pd.DataFrame], chroma_client=None) -> Dict[str, Any]:
    """Use the MCP-style assembled context and ask the LLM to return a JSON answer.
    The returned JSON should at minimum have: {answer: str, sql?: str, references: []}
    We keep the LLM prompt deterministic and ask it to expose the SQL it would run so we can validate / run it.
    """
...
# Enhanced to optionally call rag_answer_with_mcp for generation
# ---------------------------
...
def ask_argo_question(llm, emb_model, question: str, user_id: int = 0, chroma_client=None) -> Dict[str, Any]:
    ...
    if wants_nc_preferred and nc_previews:
        ...
        if combined:
            ...
            # optionally call RAG to generate textual answer
            if llm is not None:
...
# End of PART 1. Paste PART 2 after this in app.py (PART 2 contains Streamlit UI).
# ARGO RAG Explorer — PART 2: Streamlit UI (paste after PART 1)
# This UI relies on the functions defined in PART 1 (ask_argo_question, nearest_floats, etc.).
# Paste PART 1 first and then this file into a single app.py.
...
# The PART 1 module should be executed in the same file (i.e., paste PART 1 above PART 2 in app.py)
# If you prefer splitting into modules, adjust imports accordingly.
...
with tabs[3]:
    ...
    if not chat_df.empty:
        # default CHAT UX: show only table. Map optional by toggle in sidebar.
    ...
    if st.button("Ask", key="ask_btn"):
        if mode == "LLM (force)" and not have_models:
            ...
            if not question.strip():
                ...
                # Show RAG answer if available (MCP context included)
...
with tabs[4]:
    ...
    if comp_sel:
        ...
        with engine.connect() as conn:
            for fid in comp_sel:
                pattern = f"%{fid}.nc"
                res = conn.execute(text("SELECT file, date, date_update, latitude, longitude, ocean, profiler_type, institution "
